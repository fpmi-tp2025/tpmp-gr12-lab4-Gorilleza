...
bool add_region(int country_id, const char* name, long population) {
    sqlite3_stmt *stmt;
    const char *sql = "INSERT INTO regions (country_id, name, population) VALUES (?, ?, ?);";
...
bool delete_region(int id) {
    sqlite3_stmt *stmt;
    const char *sql = "DELETE FROM regions WHERE id = ?;";
...
void list_country_regions(int country_id) {
    sqlite3_stmt *stmt;
    const char *sql = "SELECT id, name, population FROM regions WHERE country_id = ?;";
...
double get_average_region_population(int country_id) {
    sqlite3_stmt *stmt;
    const char *sql = "SELECT AVG(population) FROM regions WHERE country_id = ?;";
    double avg = 0.0;
...
long get_total_population() {
    sqlite3_stmt *stmt;
    const char *sql = "SELECT SUM(population) FROM countries;";
    long total = 0;
